...
export default function ControllerView({containerState, itemState, appPrefs, onListLimitChange,
	onSearchChange, onSearchClick, onPaginationClick, onOrderBy, onOption, 
	closeModal, inputChange, goBack, session}) {

    let columns = [];
    if (itemState.prefLabels != null && itemState.prefLabels.CONTROLLER_PAGE != null) {
    	columns = itemState.prefLabels.CONTROLLER_PAGE;
    }
    let group = "TABLE1";
	...
	if (itemState.prefTexts.CONTROLLER_PAGE != null && itemState.prefTexts.CONTROLLER_PAGE.CONTROLLER_PAGE_HEADER != null) {
		header = itemState.prefTexts.CONTROLLER_PAGE.CONTROLLER_PAGE_HEADER.value;
	}
